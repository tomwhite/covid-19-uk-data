    ...
    function (data) {
      ...
      if (daily) {
        grouped = indicatorDataGrouped
          ...
          .map((d) => {
            d[1].forEach((e) => (e.value = e.dailyValue));
            return d;
          })
          // apply moving average to last 7 days for smoothing
          .map((d) => {
            d[1] = d3.zip(d[1], movingAverage(d[1], 7)).map((e) => {
              e[0].value = e[1];
              return e[0];
            });
            return d;
      ...
      const y = d3
        ...
        .domain([
          cutoff,
          getNextLogBoundary(
            d3.max(grouped, (d) => d3.max(d[1].map((e) => e.value)))
          ),
        ])
      ...
      function make_x_gridlines() {
        return d3.axisBottom(x).tickValues(getTickValuesLinear(x.domain(), 5));
      }
      svg
        .append("g")
        .attr("transform", `translate(0,${height - margin.bottom})`)
        .call(make_x_gridlines().tickSize(0));
      svg
        .append("g")
        .attr("class", "grid")
        .attr("transform", `translate(0,${height - margin.bottom})`)
      ...
      function make_y_gridlines() {
        return d3.axisLeft(y).tickValues(yTickValues);
      }
      svg
        .append("g")
        .attr("class", "grid")
      ...
      if (!daily) {
        ...
        svg
          ...
          .attr(
            ...
            d3.line()([
              [x(0), y(cutoff)],
              [x(maxX), y(doubling(cutoff, maxX, 1))],
            ])
          );
        svg
          .append("path")
          ...
          .attr(
            ...
            d3.line()([
              [x(0), y(cutoff)],
              [x(maxX), y(doubling(cutoff, maxX, 2))],
            ])
          );
        svg
          .append("path")
          ...
          .attr(
            ...
            d3.line()([
              [x(0), y(cutoff)],
              [x(maxX), y(doubling(cutoff, maxX, 3))],
            ])
          );
        svg
          .append("path")
          ...
          .attr(
            ...
            d3.line()([
              [x(0), y(cutoff)],
              [x(maxX), y(doubling(cutoff, maxX, 7))],
            ])
          );
      ...
      const indicatorLowerSingular = indicatorLower.substr(
        0,
        indicatorLower.length - 1
      );
      if (daily) {
        svg
          .append("text")
          ...
          .attr("y", 24)
          .text(
            `UK COVID-19 daily ${indicatorLower} (7 day rolling average), by number of days since ${ordinalSuffix(
              cutoff
            )} ${indicatorLowerSingular}`
          )
        ...
        svg
          ...
          .attr("y", 24)
          .text(
            `UK COVID-19 cumulative ${indicatorLower}, by number of days since ${ordinalSuffix(
              cutoff
            )} ${indicatorLowerSingular}`
          )
          .attr("font-family", "sans-serif")
...
function dailyValues(arr) {
  arr.forEach(
    (elt, i, a) => (elt.dailyValue = i == 0 ? null : elt.value - a[i - 1].value)
  );
  return arr;
}
